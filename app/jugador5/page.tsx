...
export default function Jugador5() {
  ...
  useEffect(() => {
    ...
    const savedNombre = localStorage.getItem('nombreJugador5');
    if (savedNombre) {
  ...
  useEffect(() => {
    localStorage.setItem('nombreJugador5', nombre);
  }, [nombre]);
  ...
  return (
    <div className="topcontainer">
      <div className="titulo">{nombre || 'Jugador 5'}</div> {/* Mostrar el nombre */}
      <NombreJugador jugador="jugador5" onComplete={handleNombreComplete} /> {/* Incluir el componente */}
      <div className="subtitulo">
        {currentInput === 1 && <div className="inputs"><InputJuego1 jugador="jugador5" onComplete={handleCompleteJuego1} key={`${nombre}-input1`} /></div>}
        {currentInput === 2 && <div className="inputs"><InputJuego2 jugador="jugador5" onComplete={handleCompleteJuego2} key={`${nombre}-input2`} /></div>}
        {currentInput === 3 && <div className="inputs"><InputJuego3 jugador="jugador5" onComplete={handleCompleteJuego3} key={`${nombre}-input3`} /></div>}
        {currentInput === 4 && <div className="inputs"><InputJuego4 jugador="jugador5" onComplete={handleCompleteJuego4} key={`${nombre}-input4`} /></div>}
        {currentInput === 5 && <div className="inputs"><InputJuego5 jugador="jugador5" onComplete={handleCompleteJuego5} key={`${nombre}-input5`} /></div>}
        {currentInput === 6 && <div className="inputs"><InputJuego6 jugador="jugador5" onComplete={handleCompleteJuego6} key={`${nombre}-input6`} /></div>}
        {currentInput === 7 && <div className="inputs"><InputJuego7 jugador="jugador5" onComplete={handleCompleteJuego7} key={`${nombre}-input7`} /></div>}
        {currentInput === 8 && <div className="inputs"><InputJuego8 jugador="jugador5" onComplete={handleCompleteJuego8} key={`${nombre}-input8`} /></div>}
      </div>
