...
const InputJuego7 = ({ onComplete, jugador }) => {
  const [value, setValue] = useState('');
  const [error, setError] = useState('');

  useEffect(() => {
    const savedValue = localStorage.getItem(`${jugador}-juego7`);
    if (savedValue) {
      setValue(savedValue);
    }
  }, [jugador]);
  ...
  const handleSubmit = async () => {
    if (value === '') {
      ...
      try {
        ...
        localStorage.setItem(`${jugador}-juego7`, value); // Guardar el valor en localStorage
  ...
  return (
    <div className="row g-3 align-items-center">
      <div className="col-auto">
        <label htmlFor="juego7" className="col-form-label">Juego 7</label>
      </div>
      <div className="col-auto">
        <input 
          type="text" 
