...
const InputJuego3 = ({ onComplete, jugador }) => {
  ...
  useEffect(() => {
    const savedValue = localStorage.getItem(`${jugador}-juego3`);
    if (savedValue) {
      setValue(savedValue);
    }
  }, [jugador]);
  ...
  const handleSubmit = async () => {
    if (value === '') {
      ...
      try {
        ...
        localStorage.setItem(`${jugador}-juego3`, value); // Guardar el valor en localStorage
  ...
  return (
    <div className="row g-3 align-items-center">
      <div className="col-auto">
        <label htmlFor="juego3" className="col-form-label">Juego 3</label>
      </div>
      <div className="col-auto">
        <input 
          type="text" 
