...
const InputJuego6 = ({ onComplete, jugador }) => {
  const [value, setValue] = useState('');
  const [error, setError] = useState('');

  useEffect(() => {
    const savedValue = localStorage.getItem(`${jugador}-juego8`);
    if (savedValue) {
      setValue(savedValue);
    }
  }, [jugador]);
  ...
  const handleSubmit = async () => {
    if (value === '') {
      ...
      try {
        ...
        localStorage.setItem(`${jugador}-juego8`, value); // Guardar el valor en localStorage
  ...
  return (
    <div className="row g-3 align-items-center">
      <div className="col-auto">
        <label htmlFor="juego8" className="col-form-label">Juego 8</label>
      </div>
      <div className="col-auto">
        <input 
          type="text" 
