      ...
      await runTransaction(db, async (transaction) => {
        for (const jugador in resultados) {
          if (resultados[jugador] !== '') {
            ...
            transaction.update(jugadorRef, { j2: Number(resultados[jugador]) }); // Guardar en el campo correcto 'j2'
          }
        }
      });
  ...
  return (
    <div>
      ...
      <table className="table table-bordered">
        ...
        <tbody>
          <tr>
            <td className="text-center">{jugadores.jugador2 || 'Victor'}</td>
            <td className="text-center">
              <input
                type="number"
            ...
            <td className="text-center">{jugadores.jugador6 || 'Alan'}</td>
          </tr>
          <tr>
            <td className="text-center">{jugadores.jugador4 || 'Frenky'}</td>
            <td className="text-center">
              <input
                type="number"
            ...
            <td className="text-center">{jugadores.jugador8 || 'Yuyu'}</td>
          </tr>
        </tbody>
      </table>
